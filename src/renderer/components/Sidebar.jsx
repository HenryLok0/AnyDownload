...
export default function Sidebar({ config, setConfig, isRunning }) {
...
function InputField({ label, type = "text", disabled, value, onChange, placeholder, onBrowse }) {
  return (
    <label className="block flex-1">
      ...
      <div className="flex gap-1">
        <input 
          ...
          className="w-full px-1.5 py-0.5 text-[12px] border border-[var(--border-color)] rounded-sm bg-[var(--bg-panel)] text-[var(--text-main)] outline-none focus:border-blue-500 disabled:opacity-50"
        />
        {onBrowse && (
          <button 
            type="button"
            disabled={disabled}
            onClick={onBrowse}
            className="px-2 py-0.5 text-[11px] bg-[var(--bg-header-alt)] border border-[var(--border-color)] rounded-sm hover:bg-[var(--bg-hover)] disabled:opacity-50 text-[var(--text-main)]"
          >
            ...
          </button>
        )}
